...
def Decoder(x,feedback_bits):
	...
	#x_ini = layers.Conv2D(16, 5, padding='SAME', activation='relu')(x_ini)
	x_ini = layers.Conv2D(32, 5, padding='SAME', activation='relu')(x_ini)
	for i in range(4):
		x = layers.Conv2D(16, 3, padding='SAME', activation='relu')(x_ini)
		x = layers.Conv2D(32, 3, padding='SAME')(x)

		
		#CA
		avg = layers.GlobalAveragePooling2D()(x)
		max = layers.GlobalMaxPooling2D()(x)
		avg = layers.Reshape((1, 1, avg.shape[1]))(avg)
		max = layers.Reshape((1, 1, max.shape[1]))(max)
		avg_out = layers.Conv2D(filters=in_planes, kernel_size=1, strides=1, padding='same',
								   kernel_regularizer=regularizers.l2(5e-4),
								   use_bias=True
								   )(layers.Conv2D(filters=in_planes//ratio, kernel_size=1, strides=1, padding='same',
								   kernel_regularizer=regularizers.l2(5e-4),
								   use_bias=True, activation=tf.nn.relu)(avg))
		max_out = layers.Conv2D(filters=in_planes, kernel_size=1, strides=1, padding='same',
								   kernel_regularizer=regularizers.l2(5e-4),
								   use_bias=True
								   )(layers.Conv2D(filters=in_planes//ratio, kernel_size=1, strides=1, padding='same',
								   kernel_regularizer=regularizers.l2(5e-4),
		...
		out = layers.Conv2D(filters=1, kernel_size=sa_kernel_size, strides=1, activation='sigmoid', padding='same', use_bias=False,
